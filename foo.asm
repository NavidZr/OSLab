...
int main(int argc, char *argv[])
{
   ...
   5:	89 e5                	mov    %esp,%ebp
   7:	53                   	push   %ebx
   8:	bb 05 00 00 00       	mov    $0x5,%ebx
   d:	83 e4 f0             	and    $0xfffffff0,%esp
    int pid[10];
    for (int i = 0 ; i < 5 ; i++)
    {
        pid[i] = fork();
  10:	e8 76 02 00 00       	call   28b <fork>
        if (pid[i] == 0)
  15:	85 c0                	test   %eax,%eax
  17:	74 13                	je     2c <main+0x2c>
    for (int i = 0 ; i < 5 ; i++)
  19:	83 eb 01             	sub    $0x1,%ebx
  1c:	75 f2                	jne    10 <main+0x10>
                x++;
            }
            exit();
        }
    ...
    while (wait());
  1e:	e8 78 02 00 00       	call   29b <wait>
  23:	85 c0                	test   %eax,%eax
  25:	75 f7                	jne    1e <main+0x1e>
    // for(int i=0; i<5; i++)
    //     wait();
    return 0;
...
free(void *ap)
{
 ...
 5f5:	a1 3c 0a 00 00       	mov    0xa3c,%eax
{
 ...
 616:	73 10                	jae    628 <free+0x38>
{
 ...
 641:	5b                   	pop    %ebx
  freep = p;
 642:	a3 3c 0a 00 00       	mov    %eax,0xa3c
}
 647:	5e                   	pop    %esi
 648:	5f                   	pop    %edi
 649:	5d                   	pop    %ebp
 ...
 667:	03 53 fc             	add    -0x4(%ebx),%edx
  freep = p;
 66a:	a3 3c 0a 00 00       	mov    %eax,0xa3c
    p->s.size += bp->s.size;
...
malloc(uint nbytes)
{
 ...
 68d:	8b 45 08             	mov    0x8(%ebp),%eax
  if((prevp = freep) == 0){
 690:	8b 3d 3c 0a 00 00    	mov    0xa3c,%edi
  nunits = (nbytes + sizeof(Header) - 1)/sizeof(Header) + 1;
 ...
 6d9:	8b 3d 3c 0a 00 00    	mov    0xa3c,%edi
 ...
 702:	e8 e9 fe ff ff       	call   5f0 <free>
  return freep;
 707:	a1 3c 0a 00 00       	mov    0xa3c,%eax
 ...
 734:	89 72 04             	mov    %esi,0x4(%edx)
      freep = prevp;
 737:	a3 3c 0a 00 00       	mov    %eax,0xa3c
}
 ...
 74e:	66 90                	xchg   %ax,%ax
    base.s.ptr = freep = prevp = &base;
 750:	c7 05 3c 0a 00 00 40 	movl   $0xa40,0xa3c
 757:	0a 00 00 
    base.s.size = 0;
 75a:	bf 40 0a 00 00       	mov    $0xa40,%edi
    base.s.ptr = freep = prevp = &base;
 75f:	c7 05 40 0a 00 00 40 	movl   $0xa40,0xa40
 766:	0a 00 00 
  for(p = prevp->s.ptr; ; prevp = p, p = p->s.ptr){
 769:	89 f8                	mov    %edi,%eax
    base.s.size = 0;
 76b:	c7 05 44 0a 00 00 00 	movl   $0x0,0xa44
 772:	00 00 00 
    if(p->s.size >= nunits){
